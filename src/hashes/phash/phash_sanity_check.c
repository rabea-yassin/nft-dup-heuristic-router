...
static int hash_and_report(const char *path, phash_t *out_hash) {
    int rc = phash_from_file(path, out_hash);
    if (rc != 0) {
...
int main(int argc, char **argv) {
    ...
    char path[1024];
    phash_t original, exact_copy, color_swap, background_change;
    ...
    int d_exact = phash_distance(original, exact_copy);
    int d_color = phash_distance(original, color_swap);
    int d_background = phash_distance(original, background_change);
