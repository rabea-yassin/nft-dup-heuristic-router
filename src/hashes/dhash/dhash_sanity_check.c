 ...
 * with imagehash as the parity oracle). Mirrors ahash_sanity_check.c:
 * hashes a fixed pair of images from the committed
 * data/example/generated/ fixtures and prints the resulting Hamming
 * distance: 0 for an exact copy, clearly nonzero for a visible variant. */
#include <stdio.h>
#include <stdlib.h>

#include "dhash.h"

static int hash_and_report(const char *path, dhash_t *out_hash) {
    int rc = dhash_from_file(path, out_hash);
    if (rc != 0) {
...
int main(int argc, char **argv) {
    ...
    char path[1024];
    dhash_t original, exact_copy, color_swap, background_change;
    ...
    int d_exact = dhash_distance(original, exact_copy);
    int d_color = dhash_distance(original, color_swap);
    int d_background = dhash_distance(original, background_change);
